...
class ApiResponse {
  ...
  getVersion() {
    return _parseInt__default["default"](this.response.headers.get('Last-Modified-Version'), 10);
  }
...
class SingleWriteResponse extends ApiResponse {
  ...
  getData() {
    return _objectSpread$2(_objectSpread$2({}, this.options.body), {}, {
      version: _parseInt__default["default"](this.response.headers.get('Last-Modified-Version'), 10)
    });
  }
...
class FileUploadResponse extends ApiResponse {
  ...
  getVersion() {
    return this.registerResponse ? _parseInt__default["default"](this.registerResponse.headers.get('Last-Modified-Version'), 10) : _parseInt__default["default"](this.response.headers.get('Last-Modified-Version'), 10);
  }
...
var resourcesSpecs = [//name in resource, name in the url (usually the same but there are exceptions)
{
  'name': 'library',
  urlPart: 'library',
  isKeyResource: true
...
var throwErrorResponse = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator__default["default"](function* (rawResponse, options, requestDesc) {
    ...
    } catch (_) {// unable to parse error from response but should still throw
    }
...
var request = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator__default["default"](function* (config) {
    ...
    var url = "http://".concat(options.apiAuthorityPart, "/").concat(path).concat(query);
    var fetchConfig = {};
    ...
    if (hasDefinedKey(options, 'file') && hasDefinedKey(options, 'fileName') || options.uploadRegisterOnly === true) {
      if (rawResponse.ok) {
        ...
        if ('exists' in authData && authData.exists) {
          ...
          body.set(suffix, prefix.byteLength + options.file.byteLength); // follow-up request
          ...
          if (uploadResponse.status === 201) {
            ...
            if (!registerResponse.ok) {
              yield throwErrorResponse(registerResponse, options, 'Upload stage 3: ');
            }

            response = new FileUploadResponse({}, options, rawResponse, uploadResponse, registerResponse);
          } else {
            yield throwErrorResponse(uploadResponse, options, 'Upload stage 2: ');
          }
        }
      } else {
        yield throwErrorResponse(rawResponse, options, 'Upload stage 1: ');
      }
    } else {
      if (rawResponse.status < 200 || rawResponse.status >= 400) {
        yield throwErrorResponse(rawResponse, options, '');
      }
