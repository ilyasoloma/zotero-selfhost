...
const getItemCanonicalUrl = ({ libraryKey, itemKey }) =>
	`http://192.168.62.111:8080/${libraryKey.startsWith('u') ? 'user' : 'groups'}/${libraryKey.slice(1)}/items/${itemKey}`;
...
const mapRelationsToItemKeys = (relations, libraryKey, relationType='dc:relation', shouldRemoveEmpty = true) => {
	...
	const relatedItemKeys = relatedUrls.map(relatedUrl => {
		let match = relatedUrl.match(`http?://192.168.62.111:8080/(?:users|groups)/${libraryKey.slice(1)}/items/([A-Z0-9]{8})`);
...
const isLikeURL = identifier => {
	// http://stackoverflow.com/a/3809435, modified to allow up to 9-char TLDs and IP addresses
	const urlRE = /^(http?:\/\/)?([-a-zA-Z0-9@:%._+~#=]{2,256}\.[a-z]{2,9}\b|((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|\b)){4})(\S*)$/i;
	return !!identifier.match(urlRE);
}

// http://github.com/zotero/zotero/blob/d3601bba24a83b3930711cc664e8ffad2ee41fd8/chrome/content/zotero/xpcom/utilities.js#L200
...
// http://github.com/zotero/zotero/blob/5bb2486040fa1fc617c81b4aea756ba338584f6b/chrome/content/zotero/bindings/itembox.xml#L428-L440
const getDOIURL = doi => 'http://doi.org/'
...
// http://github.com/zotero/zotero/blob/5bb2486040fa1fc617c81b4aea756ba338584f6b/chrome/content/zotero/xpcom/utilities.js#L238-L249
const cleanDOI = doi => {
